...
def main(args):
    ...
    data = Dataset(args.data_path, args.fea_type, args.Periodic_table, "formulas", "bandgap")
    args.fea_len = data[0][1].shape[-1]
    # print(args.fea_len)
    # exit()
    train, test = Subset(data, args.per, 26)
    net = DNN(args, True)
    best_r = np.zeros([3])
    ...
    with tf.Session(
        ...
        all_form, all_test, all_pre = [], [], []
        while not test.end_one_epoch:
            test_data = data[test.next_batch(args.batch_size)]
            test_x, test_y = test_data[1], test_data[-1]
            test_pre = pre(sess, net, test_x)
            all_test.append(test_y[:,np.newaxis])
            all_pre.append(test_pre)
            all_form.append(test_data[0][:,np.newaxis])

        all_test = np.concatenate(all_test)
        all_pre = np.concatenate(all_pre)
        all_form = np.concatenate(all_form)

        test_pre = np.concatenate([all_form, all_test, all_pre], axis = 1)
        file_name = args.fea_type + "_MP.csv"
        file_loss = args.fea_type + "_MP_R.csv"
        np.savetxt(file_name, test_pre, delimiter = ",", fmt = "%s")
        np.savetxt(file_loss, R_loss)
